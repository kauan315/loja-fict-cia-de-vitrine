...
function readProducts():HeroProduct[]{
  try{
    const p=JSON.parse(localStorage.getItem('urbanfit-products')||'[]')
    const f=JSON.parse(localStorage.getItem('urbanfit-featured')||'[]')
    if(!Array.isArray(p)||!Array.isArray(f))return[]
    return p.filter((x:any)=>f.includes(x.name)&&x.image).map((x:any)=>({name:String(x.name),image:String(x.image)}))
  }catch{return[]}
}
...
function setup(){
  ...
  index=0
  if(!img.dataset.heroVitrine){
    img.dataset.heroVitrine='1'
  ...
  show()
  window.clearInterval(slideTimer)
  slideTimer=window.setInterval(show,8000)
}
...
function addStyle(){
  ...
  s.id=STYLE_ID
  s.textContent='.hero-image img[data-hero-vitrine]{will-change:opacity,transform}'
...
function start(){
  ...
  refreshTimer=window.setInterval(setup,1500)
}

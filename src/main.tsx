...
function App() {
  ...
  useEffect(() => { if (featuredProducts.length > 1) { const timer = setInterval(() => setHeroIndex(i => (i + 1) % featuredProducts.length), 4000); return () => clearInterval(timer) } }, [featuredProducts.length])
        ...
        {tab === 'search' ? <SearchPage query={query} setQuery={setQuery} products={filtered} onSelect={setSelected} /> : tab === 'admin' ? <AdminPage products={products} socials={socials} setSocials={setSocials} featured={featured} setFeatured={setFeatured} /> : (
          <main>
            <section className="feature-grid">
              <div className="orange-backdrop"></div><button className="hero-image" onClick={() => heroProduct && setSelected(heroProduct)} aria-label={heroProduct ? `Ver ${heroProduct.name}` : 'Vitrine'}>{heroProduct ? <img key={heroProduct.name} src={heroProduct.image} alt={heroProduct.name}/> : <img src={heroImage} alt="Modelo Urban Fit"/>}<span className="hero-featured-label">{heroProduct ? 'DESTAQUE' : ''}</span></button><div className="hero-copy"><div className="hero-title">ROUPAS E<br/>CALÇADOS<br/>PARA O<br/>DIA A DIA</div></div>

...
function App() {
  ...
  return (
    <div className="app-shell">
      ...
      <nav className="bottom-nav"><button className={tab==='home'?'active':''} onClick={() => {setTab('home');setVitrineOpen(false)}}><Home/><span>Início</span></button><button className={tab==='search'?'active':''} onClick={() => {setTab('search');setVitrineOpen(false)}}><Search/><span>Busca</span></button><button className={tab==='admin'?'active':''} onClick={() => {setTab('admin');setVitrineOpen(false)}}><Settings2/><span>Painel ADM</span></button></nav>
    </div>
  )
}
